...
#             varX <- sqrt(allelesCorrected$fitted*(1-allelesCorrected$fitted))
#             sumVariancesSq <- (1/(t(varY)%*%varX))^2
#             sumSqVariances <- t(varY^2)%*%(varX^2)
#             varianceFactor <- 1/(sumVariancesSq*sumSqVariances)
...
#         rbind(numAlleles, chisqHW2)
    ...
    filter <- resultMatrix[,1]>5 & (numSamples*2-resultMatrix[,1])>5
    resultMatrix <- resultMatrix[filter,]
    
    obsPValues <- rep(NA, nrow(resultMatrix))
    rareIndices <- resultMatrix$NumAlleles>2 & resultMatrix$NumAlleles<=(numSamples*.04)
    lowIndices <- resultMatrix$NumAlleles>(numSamples*.04) & resultMatrix$NumAlleles<=(numSamples*.1)
    commonIndices <- resultMatrix$NumAlleles>(numSamples*.1)
    resultMatrix <- resultMatrix[,-1]
    nIt <- ncol(resultMatrix)/3 # number of phenotype risks (uniform, gradual, sharp)
    combinedResultList <- lapply(list(resultMatrix[,1:nIt], resultMatrix[,(nIt+1):(2*nIt)], resultMatrix[,(2*nIt+1):(3*nIt)]), function(results){
...
jaccardMatrix <- read.csv('~/1000GP/output_20_50/combined_jaccard.csv', row.names=1)
...
runAndPlot <- function(chr=1, correctMethod="uncorrected", ATT="ATT", subpops="all", numEigenVectors=2, numRows=5000){
    ...
    results  <- getResultDF(paste('~/1000GP/data/1000GP_Phase3_chr', chr, '.hap.gz',sep=''), phenotype= list(randomRisk, gradualRisk, sharpRisk), numRows=numRows, linesAtATime=1000, correctionVec = correction, varFactor=varFactor, sampleSubset=sampleSubset)
